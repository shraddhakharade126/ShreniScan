...
export default async function handler(req: any, res: any) {
  ...
  // Read BACKGROUND_REMOVAL_API_KEY exclusively from process.env.BACKGROUND_REMOVAL_API_KEY
  const apiKey = process.env.BACKGROUND_REMOVAL_API_KEY;
  if (!apiKey || apiKey.trim() === "") {
    console.error("Missing BACKGROUND_REMOVAL_API_KEY in server environment.");
    return sendJson(res, 500, {
      error: "BACKGROUND_REMOVAL_API_KEY environment variable is not configured on the server.",
    });
  }
